...
export class Common extends GbxChunk {
    ...
    doParseBinary(p) {
        ...
        */
        ...
        if (this.version >= 3) {
            this.addTemplateAndValue(p, "mapOrigin", "vec2");
        }
        if (this.version >= 4) {
            this.addTemplateAndValue(p, "mapTarget", "vec2");
        }
        if (this.version >= 5) {
    ...
    tm2ToNF() {
        this.deleteFromTemplate(["mapType", "mapStyle", "bool",
                "lightmapCacheUID", "lightmapVersion", "titleUID"], 5);
    }
