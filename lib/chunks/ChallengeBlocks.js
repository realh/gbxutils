...
export class ChallengeBlock extends GbxObject {
    ...
    parseBinary(p) {
        ...
        this.copyTemplate();
        if (this.version) {
            this.addTemplateAndValue(p, "flags", "uint32");
        } else {
            this.addTemplateAndValue(p, "flags", "uint16");
        }
...
export class ChallengeBlocks extends GbxChunk {
    ...
    parseBlock(p) {
        const block = new ChallengeBlock(this.version);
        block.parseBinary(p);
    ...
    doParseBinary(p) {
        ...
        while (p.bytes.length - p.offset > 4) {
            const o = p.offset;
            const w = p.uint32();
            if (w == 0xffffffff || w < 0x03000000) {
                ...
                if (flags == 0xffffffff || (!this.version && flags == 0xffff)) {
                    ...
                    throw Error(`ChallengeBlocks followed by something that ` +
