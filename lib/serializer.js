...
export class Serializer {
    ...
    // if t is not given, use each item's toBin method. If t is given it should
    // be a string corresponding to the name of the method used to write each
    // item or the item's constructor (in which case it's ignored).
    array(a, t, n) {
        if (typeof n == "number") {
            if (n != a.length) {
                throw Error(`Serializer: expected array of length ${n} ` +
                        `but actual length is ${a.length}`);
            }
        }
        for (const item of a) {
            if (typeof t == "string") {
    ...
    color(c) {
        return this.float(c.r).float(c.g).float(c.b);
    }
