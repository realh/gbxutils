...
export function getBlocksChunk(body) {
...
export function getCheckpointsChunk(body) {
...
// Makes a new array of blocks consisting of copies of the input blocks. Use
// this if you want to transform and paste a selection multiple times.
export function cloneSelection(selection) {
    return selection.map(b => b.clone());
}
...
// steps of 90 degrees wrt looking down from above, so only values of 1, 2 or 3
// make sense.
...
// This is for debugging. It returns the inclusive bounds enclosing all the
// anchor points of blocks in a selection. This does not include the extra
// space taken up by blocks which are larger than one cell.
// Returns: [minX, maxX, minY, maxY, minZ, maxZ]
export function getSelectionBounds(selection) {
    let [minX, maxX, minY, maxY, minZ, maxZ] =
        [null, null, null, null, null, null];
    for (const blk of selection) {
        if (typeof blk.x != "number" || typeof blk.y != "number" ||
                typeof blk.z != "number")
        {
            console.dir(blk);
            throw Error(`Block has invalid coords (${blk.x},${blk.y},${blk.z})`);
        }
        if (minX === null || minX > blk.x) {
            minX = blk.x;
        }
        if (maxX === null || maxX < blk.x) {
            maxX = blk.x;
        }
        if (minY === null || minY > blk.y) {
            minY = blk.y;
        }
        if (maxY === null || maxY < blk.y) {
            maxY = blk.y;
        }
        if (minZ === null || minZ > blk.z) {
            minZ = blk.z;
        }
        if (maxZ === null || maxZ < blk.z) {
            maxZ = blk.z;
        }
    }
